...
def getcalendarIdList():
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            flow = InstalledAppFlow.from_client_secrets_file(CalendarAdministratorFile, AdminDirectoryAPISCOPES)
...
def getcalendarIdList_from_txt():
    '''
    读人员邮箱地址
    '''
    try:
...
def generateEveryDayCalendarData(GmailList, paraTwoDaysAgoDate, paraYesterday):
    ...
    try:
        if GmailList:
            ...
            # 创建/RCV/STOCK/TORASINNYOU/CALENDAR/日期 文件夹
            if not os.path.exists(NormalGmailAddress + paraYesterday.replace('-','')):
                os.makedirs(NormalGmailAddress + paraYesterday.replace('-',''))
            if not os.path.exists(AbnormalGmailAddress + paraYesterday.replace('-','')):
                os.makedirs(AbnormalGmailAddress + paraYesterday.replace('-',''))
            NormalGmailAddressList = []  # 存放当前EventStartDate的正常人员的邮箱地址的列表
            AbnormalGmailAddressList = []  # 存放当前EventStartDate的异常人员的邮箱地址的列表
            for calendarId in GmailList:
                try:
                    ...
                    AbnormalGmailAddressList.append(calendarId)
                    continue
            ...
            # 保存正常人员邮箱
            with open(NormalGmailAddress + paraYesterday.replace('-', '') + os.sep + CreateTextName + NormalGmailFileName,"w", encoding="utf-8") as fo:
                fo.write('\n'.join([i for i in NormalGmailAddressList]))

            # 保存异常人员的邮箱
            with open(AbnormalGmailAddress + paraYesterday.replace('-','') + os.sep + CreateTextName + AbnormalGmailFileName,"w", encoding="utf-8") as fo:
    ...
    try:
        ...
        if len_EventDateList > 1 and paraGmailList:
            ...
            for i in range(len_EventDateList - 1):
                ...
                AbnormalGmailAddressList = [] #存放当前EventStartDate的异常人员的邮箱地址的列表
                #创建/RCV/STOCK/TORASINNYOU/CALENDAR/日期 文件夹
                if not os.path.exists(NormalGmailAddress + EventStartDate.replace('-', '')):
                    os.makedirs(NormalGmailAddress + EventStartDate.replace('-', ''))
                if not os.path.exists(AbnormalGmailAddress + EventStartDate.replace('-', '')):
                    os.makedirs(AbnormalGmailAddress + EventStartDate.replace('-', ''))
...
def read_dateConfig_file_set_parameter():
    ...
    if os.path.exists("/TORASINNYOU/TBL/GSUITE/CALENDAR/dateConfigLinux.ini"):
        try:
            conf = configparser.ConfigParser()
            conf.read( "/TORASINNYOU/TBL/GSUITE/CALENDAR/dateConfigLinux.ini", encoding="utf-8-sig")
