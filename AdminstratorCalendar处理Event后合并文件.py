            ...
            for everyDate in paraAllDateList:
                if os.path.exists(CalendarPath + everyDate + '.txt'):
                    ...
                    for item in allDataList:
                        ...
                        currentDataSaveToCSVList.append(
                            [item[79], item[17], eventStartDate, eventStartTime, eventEndDate, eventEndtTime, item[42]])

                    with open(CalendarPath + everyDate + '_SpecifiedColumns.csv', "w", newline='',
                              encoding="utf-8") as fo:
                        title = [['calendarId', 'summary', 'eventStartDate', 'eventStartTime', 'eventEndDate',
                                  'eventEndtTime', 'visibility']]
                        writer = csv.writer(fo)
                        writer.writerows(title)
        ...
        if len_EventDateList > 1 and paraGmailList:
            ...
            for i in range(len_EventDateList - 1):
                ...
                if Need_To_Save_List:
                    print(EventStartDate, EventEndDate)
                    save_txt_to_disk(CalendarPath, EventStartDate + 'event', Need_To_Save_List)
        ...
        if paraManyDays and paraGmailList:
            for x in range(paraManyDays, 1, -1):
                if x > 1:
                    ...
                    save_txt_to_disk(CalendarPath, Yesterday + 'create', Root_And_Branch_To_Txt)
    except Exception as ex:
...
def MergeEventTimeDataThenMergeCreateTimeData(paraEventDateList, paraHowManyDays):
    '''
    ...
    try:
        allEventDataList = []
        SpecifiedCSVList = [] #保存到CSV的List
        for everyDate in paraEventDateList:
            if os.path.exists(CalendarPath + everyDate + 'event.txt'):
                with open(CalendarPath + everyDate + 'event.txt', 'r', encoding='utf-8') as f:
                    lines = f.readlines()
                    for line in lines:
                        if not line:  # 如果line是空
                            continue
        ...
        with open(CalendarPath + paraEventDateList[-2] + '_SpecifiedColumns.csv', "w", newline='',
                  encoding="utf-8") as fo:
            title = [['calendarId', 'summary', 'eventStartDate', 'eventStartTime', 'eventEndDate',
                      'eventEndtTime', 'visibility']]
            writer = csv.writer(fo)
        ...
        logger.error("Call method MergeEventTimeDataThenMergeCreateTimeData() error!")
        raise ex
...
if __name__ == '__main__':
    ...
    howManyDays = 29  # 超过30天的不能取
    EventTimeMin = '2019-01-01'  # 事件的开始时间
    EventTimeMax = '2019-02-01'  # 事件的结束时间
    ...
    MergeEventTimeDataThenMergeCreateTimeData(Event_Date_List, howManyDays) #先合并事件开始时间和结束时间对应的数据,再合并创建时间(最多29天)在某段时间内的数据
    ...
    # generateAllDataByStartDateAndEndDate(Created_Updated_AllDateList)
